...
# bgl.load_data(X.drop(['P_A_most_likely','P/Na', 'protected_attribute.1','protected_attribute.2', \
#                                    'protected_attribute.3'], axis=1), y_true, noisy_sa)
...
# constraint = BoundedGroupLoss(SquareLoss(-1,1), upper_bound=1.0)

reg=LinearRegression().fit(X.drop(['P_A_most_likely','P/Na', 'protected_attribute.1','protected_attribute.2', \
                                   'protected_attribute.3','Residual'], axis=1), y_true) #training with instantiated data
mitigator = ExponentiatedGradient(reg, bgl)
...
if noisy_exp :
    for i in range(10):
        ...
        y_pred_mitigated = reg.predict(X.drop(['P_A_most_likely','P/Na','protected_attribute.1','protected_attribute.2',\
        ...
        # mae_noisy_mitigated = MetricFrame(metrics=bgl.gamma, y_true=y_true, y_pred=y_pred_mitigated, sensitive_features=noisy_sa)
    ...
    ax.set_ylabel('Loss')
    ax.set_title('Plot for Noisy Synthetic Dataset (same_mean)')
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig('noisy_regression_same_mean.png')
else:
    y_pred_mitigated = reg.predict(X.drop(['P_A_most_likely', 'P/Na','protected_attribute.1','protected_attribute.2', \
                                           'protected_attribute.3','Residual'], axis=1))
    mae_noisy_mitigated = MetricFrame(metrics=mean_absolute_error, y_true=y_true, y_pred=y_pred_mitigated, sensitive_features=noisy_sa)
    mae=[]
    mae_residual=[]
    ...
    # print('per group losses:',per_group_losses)

    for i in range(len(pa_values)):
        # diff= per_group_losses[i]-X.loc[pa_values-1,'Residual']
        # print(losses)
        print('per group losses:',per_group_losses)
        print('Residual:',X.loc[i,'Residual'])
    ...
    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Loss')
    ax.set_title('MAE vs Residual per group  for linear regression')
    ax.set_xticks(x + width, instance)
    ax.set_xlabel('Groups', fontweight ='bold', fontsize = 15)
    ax.legend(loc='best', ncol=7)
    # ax.set_ylim(0, 5.5)

    plt.tight_layout()
    plt.show()
    plt.savefig('non_noisy_regression_abs100.png')
