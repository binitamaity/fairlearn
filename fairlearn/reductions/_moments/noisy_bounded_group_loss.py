...
class NoisyConditionalLossMoment(LossMoment):
    r"""A moment for constraining the mean loss or the worst-case loss by a group,
        when the group information is noisy.
    ...
    def default_objective(self):
        ...
        return NoisyMeanLoss(self.reduction_loss)

    #Adding the P_Na arg for handling noisy data
    def load_data(self, X, y, *, sensitive_features, P_Na):
    ...
    def gamma(self, predictor):
        ...
        #expected attr for the noisy setting - multiply by P_Na
        #taking mean is same as dividing by Na
        # self.tags[_LOSS] = self.tags[_LOSS].multiply(other=self.P_Na)
...
NoisyConditionalLossMoment.__module__ = "fairlearn.reductions"


class NoisyMeanLoss(NoisyConditionalLossMoment):
...
class NoisyBoundedGroupLoss(NoisyConditionalLossMoment):
